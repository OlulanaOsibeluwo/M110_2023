...
Adafruit_DCMotor *right = AFMS.getMotor(2); //rn the motors are criss-crossed but this will change
...
int blocks_delivered = 2; //we'll change this when we're done with the second phase
...
void setup() {
  ...
  Wire.begin();
  sensor.begin(0x50);
  sensor.setMode(sensor.eSingle, sensor.eHigh); //we could try getting it to work in single capture to stop the spam
  // sensor.start();
